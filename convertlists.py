        ...
        for line in inputfile:
            if line.startswith('@@||'):
                ...
                if exceptionrule.startswith('/'):
                    ...
                    print(
                        "\tWARNING: convertlists.py does not currently support regex exceptions, and so will ignore them (rule: '{}')".format(
                            exceptionrule))
    ...
    def _domainlabels_exceptiontree_search(self, domain: str, current_node, matching_stack) -> bool:
        ...
        if rightmost_domain_label in current_node:
            ...
            if rightmost_domain_label_start == -1:
                ...
                return True
            elif self._domainlabels_exceptiontree_search(domain[:rightmost_domain_label_start],
                                                         current_node[rightmost_domain_label], matching_stack):
                return True
            matching_stack.pop()
                ...
                if fnmatch.fnmatch(rightmost_domain_label, wildcard_exception_label):
                    ...
                    if rightmost_domain_label_start == -1:
                        ...
                        return True
                                                                 ...
                                                                 current_node[wildcard_exception_label],
                                                                 matching_stack):
                        return True
                    matching_stack.pop()
...
def convertlist_adbp(inputfile, adbpconfiguration, outputfile):
    ...
    for line in inputfile:
        ...
        if line.startswith('!'):
            ...
            if exceptions.exception_affects_domain(domain):
                if verbosity >= 3:
                    print(
                        "\tINFO: Ignoring block on domain '{}' because at least one exception matches the domain, a subdomain, or a potential path on that domain or subdomain".format(
                            domain))
                domains_excluded += 1
                continue  # Skip this rule
    ...
    processinginfo.append(("Non-Domain-only Rules Excluded",
                           lines_processed - comment_lines - domains_retained - domains_excluded - domains_unsupportedoptions))
...
parser.add_argument("listsjson",
                    help="path to the input `lists.json` file, describing the lists the script should process")
parser.add_argument("outputpath",
                    help="path to a desired output folder in which the converted lists will be saved, along with a `details.json` file describing details of the conversion process and output files")
...
with open(args.listsjson, encoding='gb18030', errors='ignore') as json_data:
    ...
    for filterlist in json_object:
        ...
        if basefilename in unique_output_files:
            ...
            raise ValueError(
                "More than one list in lists.json is currently configured to use the same output filename '{!s}'. To fix this, modify the list dictionaries to specify distinct 'outputfile' filenames".format(
                    basefilename))
        ...
        if parsedurl.scheme == "http" or parsedurl.scheme == "https":
            ...
            if args.verbosity >= 1:
                print("[{!s}]: Fetch completed ({} bytes, in {:.2f} seconds)".format(filterlist['name'],
                                                                                     os.path.getsize(locallistfile),
                                                                                     (end_time - start_time)))
            ...
            if not os.path.exists(locallistfile):
                print(
                    '[{!s}]: ERROR: Input list file does not exist at path: "{!s}"; skipping'.format(filterlist['name'],
                                                                                                     filterlist['url']))
                continue
        else:
            # Unsupported scheme
            raise ValueError(
                "Unsupported url format: '{!s}'; Expecting 'https://', 'http://', or 'file://' scheme.".format(
                    filterlist['url']))
        ...
        start_time = time.monotonic()
        listdetails = dict()
        with open(os.path.join(converted_output_lists_locations, outputfilename), 'w', encoding='gb18030',
                  errors='ignore') as f:
        ...
        if args.verbosity >= 1:
            print("[{!s}]: Converted list saved to: {!s}".format(filterlist['name'],
                                                                 os.path.join(converted_output_lists_locations,
                                                                              outputfilename)))
